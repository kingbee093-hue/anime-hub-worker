...
async function getRecentMangaDexFeed() {
  ...
  for (let page = 0; page < MAX_FEED_PAGES; page++) {
    ...
    for (const chapter of chapters) {
      ...
      const mangaObj = response.data.included?.find(inc => inc.type === 'manga' && inc.id === mangaRel.id);
      if (mangaObj) {
...
async function collectFreshNewChapterItems() {
  ...
  return {
    ...
    failedItems: [], // This was added in the other file structure, but keeping consistent for this run
  };
}
